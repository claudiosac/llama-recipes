...
def main(**kwargs):
    ...
    wdb.login()
    if wandb.get("id") is not None:
        wdb.init(project=wandb.get("proj"), config=wandb.get("config"), id=wandb.get("id"), resume="must")
    else:
        wdb.init(project=wandb.get("proj"), config=wandb.get("config"))
